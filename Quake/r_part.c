...
void R_EntityParticles (entity_t *ent)
{
	...
	if (!avelocities[0][0])
	{
		...
		{
			avelocities[i][0] = (COM_Rand () & 255) * 0.01;
			avelocities[i][1] = (COM_Rand () & 255) * 0.01;
			avelocities[i][2] = (COM_Rand () & 255) * 0.01;
		}
	}
...
void R_ParticleExplosion (vec3_t org)
{
	...
	for (i = 0; i < 1024; i++)
	{
		...
		if (i & 1)
		{
			...
			for (j = 0; j < 3; j++)
			{
				p->org[j] = org[j] + ((COM_Rand () % 32) - 16);
				p->vel[j] = (COM_Rand () % 512) - 256;
			}
		}
		else
		{
...
void R_BlobExplosion (vec3_t org)
{
	...
	for (i = 0; i < 1024; i++)
	{
		...
		p->die = cl.time + 1 + (COM_Rand () & 8) * 0.05;

		if (i & 1)
		{
			p->type = pt_blob;
			p->color = 66 + COM_Rand () % 6;
			for (j = 0; j < 3; j++)
			{
				p->org[j] = org[j] + ((COM_Rand () % 32) - 16);
				p->vel[j] = (COM_Rand () % 512) - 256;
...
void R_RunParticleEffect (vec3_t org, vec3_t dir, int color, int count)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		{
			p->die = cl.time + 0.1 * (COM_Rand () % 5);
			p->color = (color & ~7) + (COM_Rand () & 7);
			p->type = pt_slowgrav;
			for (j = 0; j < 3; j++)
			{
				p->org[j] = org[j] + ((COM_Rand () & 15) - 8);
				p->vel[j] = dir[j] * 15; // + (COM_Rand()%300)-150;
			}
		}
	}
}
	...
	for (i = -16; i < 16; i++)
		for (j = -16; j < 16; j++)
			for (k = 0; k < 1; k++)
			{
				...
				p->die = cl.time + 2 + (COM_Rand () & 31) * 0.02;
				p->color = 224 + (COM_Rand () & 7);
				p->type = pt_slowgrav;

				dir[0] = j * 8 + (COM_Rand () & 7);
				dir[1] = i * 8 + (COM_Rand () & 7);
				dir[2] = 256;

				p->org[0] = org[0] + dir[0];
				p->org[1] = org[1] + dir[1];
				p->org[2] = org[2] + (COM_Rand () & 63);
	...
	for (i = -16; i < 16; i += 4)
		for (j = -16; j < 16; j += 4)
			for (k = -24; k < 32; k += 4)
			{
				...
				p->die = cl.time + 0.2 + (COM_Rand () & 7) * 0.02;
				p->color = 7 + (COM_Rand () & 7);
				p->type = pt_slowgrav;
				...
				p->org[0] = org[0] + i + (COM_Rand () & 3);
				p->org[1] = org[1] + j + (COM_Rand () & 3);
				p->org[2] = org[2] + k + (COM_Rand () & 3);
...
void R_RocketTrail (vec3_t start, vec3_t end, int type)
{
	...
	while (len > 0)
	{
		...
		case 6: // voor trail
			p->color = 9 * 16 + 8 + (COM_Rand () & 3);
			p->type = pt_static;
			p->die = cl.time + 0.3;
			for (j = 0; j < 3; j++)
				p->org[j] = start[j] + ((COM_Rand () & 15) - 8);
			break;
		}
...
void R_DrawParticles (cb_context_t *cbx)
{
	...
	if (cbx->render_pass_index == RENDER_PASS_INDEX_MAIN_OIT)
		...
		pipeline = vulkan_globals.particle_wavelet_bounds_pipeline;
