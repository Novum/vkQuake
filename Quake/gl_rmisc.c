...
void R_BindWaveletCoefficients (cb_context_t *cbx, uint32_t set_index)
{
	vulkan_globals.vk_cmd_bind_descriptor_sets (
		cbx->cb, VK_PIPELINE_BIND_POINT_GRAPHICS, cbx->current_pipeline.layout.handle, set_index, 1, &vulkan_globals.wavelet_coeff_descriptor_set, 0, NULL);
}
...
void R_CreateDescriptorSetLayouts ()
{
...

	{
		...
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateDescriptorSetLayout failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.wavelet_coeff_set_layout.handle, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, "wavelet coeff storage image");
	}
...
void R_CreatePipelineLayouts ()
{
...

	{
		...
		VkDescriptorSetLayout basic_wavelet_descriptor_set_layouts[2] = {
			vulkan_globals.single_texture_set_layout.handle,
			vulkan_globals.wavelet_coeff_set_layout.handle,
		};
		...
		pipeline_layout_create_info.setLayoutCount = countof (basic_wavelet_descriptor_set_layouts);
		pipeline_layout_create_info.pSetLayouts = basic_wavelet_descriptor_set_layouts;
		err = vkCreatePipelineLayout (vulkan_globals.device, &pipeline_layout_create_info, NULL, &vulkan_globals.basic_wavelet_pipeline_layout.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreatePipelineLayout failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.basic_wavelet_pipeline_layout.handle, VK_OBJECT_TYPE_PIPELINE_LAYOUT, "basic_wavelet_pipeline_layout");
		vulkan_globals.basic_wavelet_pipeline_layout.push_constant_range = push_constant_range;
	}

	{
		...
		VkDescriptorSetLayout world_wavelet_descriptor_set_layouts[4] = {
			vulkan_globals.single_texture_set_layout.handle, vulkan_globals.single_texture_set_layout.handle, vulkan_globals.single_texture_set_layout.handle,
			vulkan_globals.wavelet_coeff_set_layout.handle};
		...
		pipeline_layout_create_info.setLayoutCount = countof (world_wavelet_descriptor_set_layouts);
		pipeline_layout_create_info.pSetLayouts = world_wavelet_descriptor_set_layouts;
		err = vkCreatePipelineLayout (vulkan_globals.device, &pipeline_layout_create_info, NULL, &vulkan_globals.world_wavelet_pipeline_layout.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreatePipelineLayout failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.world_wavelet_pipeline_layout.handle, VK_OBJECT_TYPE_PIPELINE_LAYOUT, "world_wavelet_pipeline_layout");
		vulkan_globals.world_wavelet_pipeline_layout.push_constant_range = push_constant_range;
	}

	{
		...
		VkDescriptorSetLayout alias_wavelet_descriptor_set_layouts[4] = {
			vulkan_globals.single_texture_set_layout.handle, vulkan_globals.single_texture_set_layout.handle, vulkan_globals.ubo_set_layout.handle,
			vulkan_globals.wavelet_coeff_set_layout.handle};
		...
		pipeline_layout_create_info.setLayoutCount = countof (alias_wavelet_descriptor_set_layouts);
		pipeline_layout_create_info.pSetLayouts = alias_wavelet_descriptor_set_layouts;
		err = vkCreatePipelineLayout (vulkan_globals.device, &pipeline_layout_create_info, NULL, &vulkan_globals.alias_wavelet_pipeline_layout.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreatePipelineLayout failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.alias_wavelet_pipeline_layout.handle, VK_OBJECT_TYPE_PIPELINE_LAYOUT, "alias_wavelet_pipeline_layout");
		vulkan_globals.alias_wavelet_pipeline_layout.push_constant_range = push_constant_range;
	}

	{
		...
		VkDescriptorSetLayout md5_wavelet_descriptor_set_layouts[5] = {
			vulkan_globals.single_texture_set_layout.handle, vulkan_globals.single_texture_set_layout.handle, vulkan_globals.ubo_set_layout.handle,
			vulkan_globals.joints_buffer_set_layout.handle, vulkan_globals.wavelet_coeff_set_layout.handle};
		...
		pipeline_layout_create_info.setLayoutCount = countof (md5_wavelet_descriptor_set_layouts);
		pipeline_layout_create_info.pSetLayouts = md5_wavelet_descriptor_set_layouts;
		err = vkCreatePipelineLayout (vulkan_globals.device, &pipeline_layout_create_info, NULL, &vulkan_globals.md5_wavelet_pipeline_layout.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreatePipelineLayout failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.md5_wavelet_pipeline_layout.handle, VK_OBJECT_TYPE_PIPELINE_LAYOUT, "md5_wavelet_pipeline_layout");
		vulkan_globals.md5_wavelet_pipeline_layout.push_constant_range = push_constant_range;
	}
...
static uint32_t R_MainColorAttachmentCount (qboolean use_oit)
{
	return use_oit ? 3 : 1;
}
...
static void R_InitDefaultStates (pipeline_create_infos_t *infos)
{
	...
	infos->color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
...
static void R_SetAdditiveBlend (VkPipelineColorBlendAttachmentState *blend_state)
{
...
static void R_SetDestinationColorModulateBlend (VkPipelineColorBlendAttachmentState *blend_state)
{
	blend_state->blendEnable = VK_TRUE;
...
static void R_CreateBasicPipelines ()
{
	...
	const struct
	{
		...
		{RENDER_PASS_INDEX_MAIN, main_render_pass, vulkan_globals.sample_count, R_MainColorAttachmentCount (false)},
		{RENDER_PASS_INDEX_UI, ui_render_pass, VK_SAMPLE_COUNT_1_BIT, 1},
		{RENDER_PASS_INDEX_MAIN_OIT, main_oit_render_pass, vulkan_globals.sample_count, R_MainColorAttachmentCount (true)},
	};
...
static void R_CreateParticlesPipelines ()
{
	...
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	...
	infos.blend_attachment_states[1].blendEnable = VK_TRUE;
	infos.blend_attachment_states[1].colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
	infos.blend_attachment_states[1].srcColorBlendFactor = VK_BLEND_FACTOR_ONE;
	infos.blend_attachment_states[1].dstColorBlendFactor = VK_BLEND_FACTOR_ONE;
	infos.blend_attachment_states[1].colorBlendOp = VK_BLEND_OP_ADD;
	infos.blend_attachment_states[1].srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE;
	infos.blend_attachment_states[1].dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE;
	infos.blend_attachment_states[1].alphaBlendOp = VK_BLEND_OP_ADD;
	infos.blend_attachment_states[2].blendEnable = VK_TRUE;
	infos.blend_attachment_states[2].colorWriteMask = VK_COLOR_COMPONENT_R_BIT;
	infos.blend_attachment_states[2].srcColorBlendFactor = VK_BLEND_FACTOR_ZERO;
	infos.blend_attachment_states[2].dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR;
	infos.blend_attachment_states[2].colorBlendOp = VK_BLEND_OP_ADD;
	infos.blend_attachment_states[2].srcAlphaBlendFactor = VK_BLEND_FACTOR_ZERO;
	infos.blend_attachment_states[2].dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
	infos.blend_attachment_states[2].alphaBlendOp = VK_BLEND_OP_ADD;
	...
	if (!vulkan_globals.device_features.fragmentStoresAndAtomics)
		return;
	...
	err = vkCreateGraphicsPipelines (
		vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.particle_wavelet_coeff_pipeline.handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (particle_wavelet_coeff_pipeline) with code %i", (int)err);
	vulkan_globals.particle_wavelet_coeff_pipeline.layout = vulkan_globals.basic_wavelet_pipeline_layout;
	GL_SetObjectName ((uint64_t)vulkan_globals.particle_wavelet_coeff_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "particles_wavelet_coeff");

	infos.graphics_pipeline.renderPass = vulkan_globals.wavelet_shade_render_pass;
	infos.color_blend_state.attachmentCount = 1;
	infos.shader_stages[1].module = basic_wavelet_shade_frag_module;
	...
	assert (vulkan_globals.particle_wavelet_shade_pipeline.handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (
		vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.particle_wavelet_shade_pipeline.handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (particle_wavelet_shade_pipeline) with code %i", (int)err);
	vulkan_globals.particle_wavelet_shade_pipeline.layout = vulkan_globals.basic_wavelet_pipeline_layout;
	GL_SetObjectName ((uint64_t)vulkan_globals.particle_wavelet_shade_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "particles_wavelet_shade");
}
...
static void R_CreateFTEParticlesPipelines ()
{
	...
	for (int i = 0; i < 8; ++i)
	{
		infos.input_assembly_state.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
		infos.rasterization_state.polygonMode = VK_POLYGON_MODE_FILL;
		infos.blend_attachment_states[0].srcColorBlendFactor = source_blend_factors[i];
		infos.blend_attachment_states[0].dstColorBlendFactor = dest_blend_factors[i];
		infos.blend_attachment_states[0].colorBlendOp = VK_BLEND_OP_ADD;
		infos.blend_attachment_states[0].srcAlphaBlendFactor = source_blend_factors[i];
		infos.blend_attachment_states[0].dstAlphaBlendFactor = source_blend_factors[i];
		...
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (fte_particle_pipelines[%d]) with code %i", i, (int)err);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.fte_particle_oit_pipelines[i].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.fte_particle_oit_pipelines[i].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (fte_particle_oit_pipelines[%d]) with code %i", i, (int)err);
		vulkan_globals.fte_particle_oit_pipelines[i].layout = vulkan_globals.basic_pipeline_layout;
		GL_SetObjectName (
			(uint64_t)vulkan_globals.fte_particle_oit_pipelines[i].handle, VK_OBJECT_TYPE_PIPELINE, va ("%s_oit", fte_particle_pipeline_names[i]));
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		if (vulkan_globals.non_solid_fill)
		{
			...
			if (err != VK_SUCCESS)
				Sys_Error ("vkCreateGraphicsPipelines failed (vulkan_globals.fte_particle_pipelines[%d]) with code %i", i + 8, (int)err);
			...
			infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
			infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
			assert (vulkan_globals.fte_particle_oit_pipelines[i + 8].handle == VK_NULL_HANDLE);
			err = vkCreateGraphicsPipelines (
				vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.fte_particle_oit_pipelines[i + 8].handle);
			if (err != VK_SUCCESS)
				Sys_Error ("vkCreateGraphicsPipelines failed (vulkan_globals.fte_particle_oit_pipelines[%d]) with code %i", i + 8, (int)err);
			vulkan_globals.fte_particle_oit_pipelines[i + 8].layout = vulkan_globals.basic_pipeline_layout;
			GL_SetObjectName (
				(uint64_t)vulkan_globals.fte_particle_oit_pipelines[i + 8].handle, VK_OBJECT_TYPE_PIPELINE, va ("%s_oit", fte_particle_pipeline_names[i + 8]));
			infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
			infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		}
	}
...
static void R_CreateSpritesPipelines ()
{
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (sprite_pipeline) with code %i", (int)err);
	...
	err = vkCreateGraphicsPipelines (
		vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sprite_wavelet_coeff_pipeline.handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (sprite_wavelet_coeff_pipeline) with code %i", (int)err);
	vulkan_globals.sprite_wavelet_coeff_pipeline.layout = vulkan_globals.basic_wavelet_pipeline_layout;
	GL_SetObjectName ((uint64_t)vulkan_globals.sprite_wavelet_coeff_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "sprite_wavelet_coeff");
	...
	assert (vulkan_globals.sprite_wavelet_shade_pipeline.handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (
		vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sprite_wavelet_shade_pipeline.handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (sprite_wavelet_shade_pipeline) with code %i", (int)err);
	vulkan_globals.sprite_wavelet_shade_pipeline.layout = vulkan_globals.basic_wavelet_pipeline_layout;
	GL_SetObjectName ((uint64_t)vulkan_globals.sprite_wavelet_shade_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "sprite_wavelet_shade");
}
...
static void R_CreateSkyPipelines ()
{
	for (int i = 0; i < 2; i++)
	{
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.sky_stencil_oit_pipeline[i].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sky_stencil_oit_pipeline[i].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (sky_stencil_oit_pipeline) with code %i", (int)err);
		vulkan_globals.sky_stencil_oit_pipeline[i].layout = vulkan_globals.sky_pipeline_layout[0];
		GL_SetObjectName (
			(uint64_t)vulkan_globals.sky_stencil_oit_pipeline[i].handle, VK_OBJECT_TYPE_PIPELINE, i ? "sky_stencil_indirect_oit" : "sky_stencil_oit");
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.sky_color_oit_pipeline[i].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sky_color_oit_pipeline[i].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (sky_color_oit_pipeline) with code %i", (int)err);
		vulkan_globals.sky_color_oit_pipeline[i].layout = vulkan_globals.sky_pipeline_layout[0];
		GL_SetObjectName ((uint64_t)vulkan_globals.sky_color_oit_pipeline[i].handle, VK_OBJECT_TYPE_PIPELINE, i ? "sky_color_indirect_oit" : "sky_color_oit");
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.sky_cube_oit_pipeline[i].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sky_cube_oit_pipeline[i].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (sky_cube_oit_pipeline) with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.sky_cube_oit_pipeline[i].handle, VK_OBJECT_TYPE_PIPELINE, i ? "sky_cube_indirect_oit" : "sky_cube_oit");
		vulkan_globals.sky_cube_oit_pipeline[i].layout = vulkan_globals.sky_pipeline_layout[0];
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.sky_layer_oit_pipeline[i].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sky_layer_oit_pipeline[i].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (sky_layer_oit_pipeline) with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.sky_layer_oit_pipeline[i].handle, VK_OBJECT_TYPE_PIPELINE, i ? "sky_layer_indirect_oit" : "sky_layer_oit");
		vulkan_globals.sky_layer_oit_pipeline[i].layout = vulkan_globals.sky_pipeline_layout[1];
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.sky_box_oit_pipeline.handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.sky_box_oit_pipeline.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (sky_box_oit_pipeline) with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.sky_box_oit_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "sky_box_oit");
		vulkan_globals.sky_box_oit_pipeline.layout = vulkan_globals.sky_pipeline_layout[0];
	}
}
...
static void R_CreateShowTrisPipelines ()
{
	...
	if (vulkan_globals.non_solid_fill)
	{
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.showtris_oit_pipeline.handle == VK_NULL_HANDLE);
		err =
			vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.showtris_oit_pipeline.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (showtris_oit_pipeline) with code %i", (int)err);
		vulkan_globals.showtris_oit_pipeline.layout = vulkan_globals.basic_pipeline_layout;
		GL_SetObjectName ((uint64_t)vulkan_globals.showtris_oit_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "showtris_oit");
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.showtris_depth_test_oit_pipeline.handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.showtris_depth_test_oit_pipeline.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (showtris_depth_test_oit_pipeline) with code %i", (int)err);
		vulkan_globals.showtris_depth_test_oit_pipeline.layout = vulkan_globals.basic_pipeline_layout;
		GL_SetObjectName ((uint64_t)vulkan_globals.showtris_depth_test_oit_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "showtris_depth_test_oit");
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.showbboxes_oit_pipeline.handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.showbboxes_oit_pipeline.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (showbboxes_oit_pipeline) with code %i", (int)err);
		vulkan_globals.showbboxes_oit_pipeline.layout = vulkan_globals.basic_pipeline_layout;
		GL_SetObjectName ((uint64_t)vulkan_globals.showbboxes_oit_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "showbboxes_oit");
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.showtris_indirect_oit_pipeline.handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.showtris_indirect_oit_pipeline.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (showtris_indirect_oit_pipeline) with code %i", (int)err);
		vulkan_globals.showtris_indirect_oit_pipeline.layout = vulkan_globals.basic_pipeline_layout;
		GL_SetObjectName ((uint64_t)vulkan_globals.showtris_indirect_oit_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "showtris_indirect_oit");
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.showtris_indirect_depth_test_oit_pipeline.handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.showtris_indirect_depth_test_oit_pipeline.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (showtris_indirect_depth_test_oit_pipeline) with code %i", (int)err);
		vulkan_globals.showtris_indirect_depth_test_oit_pipeline.layout = vulkan_globals.basic_pipeline_layout;
		GL_SetObjectName (
			(uint64_t)vulkan_globals.showtris_indirect_depth_test_oit_pipeline.handle, VK_OBJECT_TYPE_PIPELINE, "showtris_indirect_depth_test_oit");
	}
}
		...
		for (alpha_test = 0; alpha_test < 2; ++alpha_test)
		{
			for (fullbright_enabled = 0; fullbright_enabled < 2; ++fullbright_enabled)
			{
				for (quantize_lm = 0; quantize_lm < 2; ++quantize_lm)
				{
					...
					if (err != VK_SUCCESS)
						Sys_Error ("vkCreateGraphicsPipelines failed (world_pipelines[%d]) with code %i", pipeline_index, (int)err);
					GL_SetObjectName (
					...
					if (alpha_blend)
					{
						...
						infos.blend_attachment_states[1].blendEnable = VK_TRUE;
						infos.blend_attachment_states[1].colorWriteMask =
							VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
						infos.blend_attachment_states[1].srcColorBlendFactor = VK_BLEND_FACTOR_ONE;
						infos.blend_attachment_states[1].dstColorBlendFactor = VK_BLEND_FACTOR_ONE;
						infos.blend_attachment_states[1].colorBlendOp = VK_BLEND_OP_ADD;
					...
					assert (vulkan_globals.world_oit_pipelines[pipeline_index].handle == VK_NULL_HANDLE);
					err = vkCreateGraphicsPipelines (
						vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.world_oit_pipelines[pipeline_index].handle);
					if (err != VK_SUCCESS)
						Sys_Error ("vkCreateGraphicsPipelines failed (world_oit_pipelines[%d]) with code %i", pipeline_index, (int)err);
					GL_SetObjectName (
						(uint64_t)vulkan_globals.world_oit_pipelines[pipeline_index].handle, VK_OBJECT_TYPE_PIPELINE, va ("world_oit %d", pipeline_index));
					vulkan_globals.world_oit_pipelines[pipeline_index].layout = vulkan_globals.world_pipeline_layout;
					...
					if (vulkan_globals.device_features.fragmentStoresAndAtomics)
					{
						...
						assert (vulkan_globals.world_wavelet_coeff_pipelines[pipeline_index].handle == VK_NULL_HANDLE);
						err = vkCreateGraphicsPipelines (
							vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL,
							&vulkan_globals.world_wavelet_coeff_pipelines[pipeline_index].handle);
						if (err != VK_SUCCESS)
							Sys_Error ("vkCreateGraphicsPipelines failed (world_wavelet_coeff_pipelines[%d]) with code %i", pipeline_index, (int)err);
						GL_SetObjectName (
							(uint64_t)vulkan_globals.world_wavelet_coeff_pipelines[pipeline_index].handle, VK_OBJECT_TYPE_PIPELINE,
							va ("world_wavelet_coeff %d", pipeline_index));
						vulkan_globals.world_wavelet_coeff_pipelines[pipeline_index].layout = vulkan_globals.world_wavelet_pipeline_layout;

						infos.graphics_pipeline.renderPass = vulkan_globals.wavelet_shade_render_pass;
						infos.color_blend_state.attachmentCount = 1;
						infos.shader_stages[1].module = world_wavelet_shade_frag_module;
						if (alpha_blend)
							R_SetAdditiveBlend (&infos.blend_attachment_states[0]);
						else
							infos.blend_attachment_states[0].blendEnable = VK_FALSE;
						...
						assert (vulkan_globals.world_wavelet_shade_pipelines[pipeline_index].handle == VK_NULL_HANDLE);
						err = vkCreateGraphicsPipelines (
							vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL,
							&vulkan_globals.world_wavelet_shade_pipelines[pipeline_index].handle);
						if (err != VK_SUCCESS)
							Sys_Error ("vkCreateGraphicsPipelines failed (world_wavelet_shade_pipelines[%d]) with code %i", pipeline_index, (int)err);
						GL_SetObjectName (
							(uint64_t)vulkan_globals.world_wavelet_shade_pipelines[pipeline_index].handle, VK_OBJECT_TYPE_PIPELINE,
							va ("world_wavelet_shade %d", pipeline_index));
						vulkan_globals.world_wavelet_shade_pipelines[pipeline_index].layout = vulkan_globals.world_wavelet_pipeline_layout;
					}
...
static void R_CreateAliasPipelines ()
{
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.alias_pipelines[0].handle, VK_OBJECT_TYPE_PIPELINE, "alias");
	vulkan_globals.alias_pipelines[0].layout = vulkan_globals.alias_pipelines[0].layout;

	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	assert (vulkan_globals.alias_oit_pipelines[0].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_oit_pipelines[0].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_oit_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.alias_oit_pipelines[0].handle, VK_OBJECT_TYPE_PIPELINE, "alias_oit");
	vulkan_globals.alias_oit_pipelines[0].layout = vulkan_globals.alias_pipelines[0].layout;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_alphatest_pipeline) with code %i", (int)err);
	...
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	assert (vulkan_globals.alias_oit_pipelines[1].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_oit_pipelines[1].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_oit_alphatest_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.alias_oit_pipelines[1].handle, VK_OBJECT_TYPE_PIPELINE, "alias_oit_alphatest");
	vulkan_globals.alias_oit_pipelines[1].layout = vulkan_globals.alias_pipelines[0].layout;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_blend_pipeline) with code %i", (int)err);
	...
	infos.blend_attachment_states[2].dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
	infos.blend_attachment_states[2].alphaBlendOp = VK_BLEND_OP_ADD;
	infos.shader_stages[1].module = alias_oit_frag_module;

	assert (vulkan_globals.alias_oit_pipelines[2].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_oit_pipelines[2].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_oit_blend_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.alias_oit_pipelines[2].handle, VK_OBJECT_TYPE_PIPELINE, "alias_oit_blend");
	vulkan_globals.alias_oit_pipelines[2].layout = vulkan_globals.alias_pipelines[0].layout;
	infos.shader_stages[1].module = alias_frag_module;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	infos.blend_attachment_states[2].dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
	infos.blend_attachment_states[2].alphaBlendOp = VK_BLEND_OP_ADD;
	infos.shader_stages[1].module = alias_alphatest_oit_frag_module;

	assert (vulkan_globals.alias_oit_pipelines[3].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_oit_pipelines[3].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (alias_oit_alphatest_blend_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.alias_oit_pipelines[3].handle, VK_OBJECT_TYPE_PIPELINE, "alias_oit_alphatest_blend");
	vulkan_globals.alias_oit_pipelines[3].layout = vulkan_globals.alias_pipelines[0].layout;
	infos.shader_stages[1].module = alias_alphatest_frag_module;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	if (vulkan_globals.non_solid_fill)
	{
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.alias_oit_pipelines[4].handle == VK_NULL_HANDLE);
		err =
			vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_oit_pipelines[4].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.alias_oit_pipelines[4].handle, VK_OBJECT_TYPE_PIPELINE, "alias_showtris_oit");
		vulkan_globals.alias_oit_pipelines[4].layout = vulkan_globals.alias_pipelines[0].layout;
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.alias_oit_pipelines[5].handle == VK_NULL_HANDLE);
		err =
			vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_oit_pipelines[5].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.alias_oit_pipelines[5].handle, VK_OBJECT_TYPE_PIPELINE, "alias_showtris_depth_test_oit");
		vulkan_globals.alias_oit_pipelines[5].layout = vulkan_globals.alias_pipelines[0].layout;
	}
	...
	infos.vertex_input_state.pVertexBindingDescriptions = alias_vertex_binding_descriptions;
	infos.shader_stages[0].module = alias_vert_module;
	infos.graphics_pipeline.layout = vulkan_globals.alias_wavelet_pipeline_layout.handle;

	for (int pipeline_index = 0; pipeline_index < 4; ++pipeline_index)
	{
		const qboolean alpha_test = (pipeline_index & 1) != 0;
		const qboolean alpha_blend = (pipeline_index & 2) != 0;
		...
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_wavelet_coeff_pipelines[pipeline_index].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (alias_wavelet_coeff_pipelines[%d]) with code %i", pipeline_index, (int)err);
		GL_SetObjectName (
			(uint64_t)vulkan_globals.alias_wavelet_coeff_pipelines[pipeline_index].handle, VK_OBJECT_TYPE_PIPELINE,
			va ("alias_wavelet_coeff %d", pipeline_index));
		vulkan_globals.alias_wavelet_coeff_pipelines[pipeline_index].layout = vulkan_globals.alias_wavelet_pipeline_layout;
		...
		if (alpha_blend)
			...
			infos.blend_attachment_states[0].blendEnable = VK_FALSE;
		infos.shader_stages[1].module = alpha_test ? alias_alphatest_wavelet_shade_frag_module : alias_wavelet_shade_frag_module;

		assert (vulkan_globals.alias_wavelet_shade_pipelines[pipeline_index].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (
			vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.alias_wavelet_shade_pipelines[pipeline_index].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (alias_wavelet_shade_pipelines[%d]) with code %i", pipeline_index, (int)err);
		GL_SetObjectName (
			(uint64_t)vulkan_globals.alias_wavelet_shade_pipelines[pipeline_index].handle, VK_OBJECT_TYPE_PIPELINE,
			va ("alias_wavelet_shade %d", pipeline_index));
		vulkan_globals.alias_wavelet_shade_pipelines[pipeline_index].layout = vulkan_globals.alias_wavelet_pipeline_layout;
	}
}
...
static void R_CreateMD5Pipelines ()
{
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.md5_pipelines[0].handle, VK_OBJECT_TYPE_PIPELINE, "md5");
	vulkan_globals.md5_pipelines[0].layout = vulkan_globals.md5_pipelines[0].layout;

	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	assert (vulkan_globals.md5_oit_pipelines[0].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.md5_oit_pipelines[0].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_oit_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.md5_oit_pipelines[0].handle, VK_OBJECT_TYPE_PIPELINE, "md5_oit");
	vulkan_globals.md5_oit_pipelines[0].layout = vulkan_globals.md5_pipelines[0].layout;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_alphatest_pipeline) with code %i", (int)err);
	...
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	assert (vulkan_globals.md5_oit_pipelines[1].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.md5_oit_pipelines[1].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_oit_alphatest_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.md5_oit_pipelines[1].handle, VK_OBJECT_TYPE_PIPELINE, "md5_oit_alphatest");
	vulkan_globals.md5_oit_pipelines[1].layout = vulkan_globals.md5_pipelines[0].layout;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_blend_pipeline) with code %i", (int)err);
	...
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	assert (vulkan_globals.md5_oit_pipelines[2].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.md5_oit_pipelines[2].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_oit_blend_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.md5_oit_pipelines[2].handle, VK_OBJECT_TYPE_PIPELINE, "md5_oit_blend");
	vulkan_globals.md5_oit_pipelines[2].layout = vulkan_globals.md5_pipelines[0].layout;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
	assert (vulkan_globals.md5_oit_pipelines[3].handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.md5_oit_pipelines[3].handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkCreateGraphicsPipelines failed (md5_oit_alphatest_blend_pipeline) with code %i", (int)err);
	GL_SetObjectName ((uint64_t)vulkan_globals.md5_oit_pipelines[3].handle, VK_OBJECT_TYPE_PIPELINE, "md5_oit_alphatest_blend");
	vulkan_globals.md5_oit_pipelines[3].layout = vulkan_globals.md5_pipelines[0].layout;
	infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
	infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
	...
	if (vulkan_globals.non_solid_fill)
	{
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.md5_oit_pipelines[4].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.md5_oit_pipelines[4].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (md5_oit_showtris_pipeline) with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.md5_oit_pipelines[4].handle, VK_OBJECT_TYPE_PIPELINE, "md5_oit_showtris");
		vulkan_globals.md5_oit_pipelines[4].layout = vulkan_globals.md5_pipelines[0].layout;
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_STANDARD][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (false);
		...
		infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_OIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
		infos.color_blend_state.attachmentCount = R_MainColorAttachmentCount (true);
		assert (vulkan_globals.md5_oit_pipelines[5].handle == VK_NULL_HANDLE);
		err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.md5_oit_pipelines[5].handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateGraphicsPipelines failed (md5_oit_showtris_depth_test_pipeline) with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.md5_oit_pipelines[5].handle, VK_OBJECT_TYPE_PIPELINE, "md5_oit_showtris_depth_test");
		vulkan_globals.md5_oit_pipelines[5].layout = vulkan_globals.md5_pipelines[0].layout;
	}
}
...
static void R_CreatePostprocessPipelines ()
{
	...
	infos.graphics_pipeline.layout = vulkan_globals.basic_wavelet_pipeline_layout.handle;
	infos.graphics_pipeline.subpass = 0;
	infos.shader_stages[1].module = wavelet_resolve_frag_module;
	R_SetDestinationColorModulateBlend (&infos.blend_attachment_states[0]);

	assert (vulkan_globals.wavelet_resolve_pipeline.handle == VK_NULL_HANDLE);
	err = vkCreateGraphicsPipelines (vulkan_globals.device, VK_NULL_HANDLE, 1, &infos.graphics_pipeline, NULL, &vulkan_globals.wavelet_resolve_pipeline.handle);
	if (err != VK_SUCCESS)
...
void R_DestroyPipelines (void)
{
	...
	for (i = 0; i < WORLD_PIPELINE_COUNT; ++i)
	{
		...
		if (vulkan_globals.world_oit_pipelines[i].handle != VK_NULL_HANDLE)
		{
		...
		if (vulkan_globals.world_wavelet_coeff_pipelines[i].handle != VK_NULL_HANDLE)
		{
			...
			vulkan_globals.world_wavelet_coeff_pipelines[i].handle = VK_NULL_HANDLE;
		}
		if (vulkan_globals.world_wavelet_shade_pipelines[i].handle != VK_NULL_HANDLE)
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.world_wavelet_shade_pipelines[i].handle, NULL);
			vulkan_globals.world_wavelet_shade_pipelines[i].handle = VK_NULL_HANDLE;
		}
	}
	...
	{
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.particle_wavelet_coeff_pipeline.handle, NULL);
		vulkan_globals.particle_wavelet_coeff_pipeline.handle = VK_NULL_HANDLE;
	}
	if (vulkan_globals.particle_wavelet_shade_pipeline.handle != VK_NULL_HANDLE)
	{
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.particle_wavelet_shade_pipeline.handle, NULL);
		vulkan_globals.particle_wavelet_shade_pipeline.handle = VK_NULL_HANDLE;
	}
#ifdef PSET_SCRIPT
	for (i = 0; i < 8; ++i)
	{
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.fte_particle_oit_pipelines[i].handle, NULL);
		vulkan_globals.fte_particle_oit_pipelines[i].handle = VK_NULL_HANDLE;
		if (vulkan_globals.non_solid_fill)
		{
			...
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.fte_particle_oit_pipelines[i + 8].handle, NULL);
			vulkan_globals.fte_particle_oit_pipelines[i + 8].handle = VK_NULL_HANDLE;
		}
	}
#endif
	...
	if (vulkan_globals.sprite_wavelet_coeff_pipeline.handle != VK_NULL_HANDLE)
	{
		...
		vulkan_globals.sprite_wavelet_coeff_pipeline.handle = VK_NULL_HANDLE;
	}
	if (vulkan_globals.sprite_wavelet_shade_pipeline.handle != VK_NULL_HANDLE)
	{
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.sprite_wavelet_shade_pipeline.handle, NULL);
		vulkan_globals.sprite_wavelet_shade_pipeline.handle = VK_NULL_HANDLE;
	}
	...
	for (i = 0; i < 2; ++i)
	{
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.sky_stencil_oit_pipeline[i].handle, NULL);
		vulkan_globals.sky_stencil_oit_pipeline[i].handle = VK_NULL_HANDLE;
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.sky_color_oit_pipeline[i].handle, NULL);
		vulkan_globals.sky_color_oit_pipeline[i].handle = VK_NULL_HANDLE;
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.sky_cube_oit_pipeline[i].handle, NULL);
		vulkan_globals.sky_cube_oit_pipeline[i].handle = VK_NULL_HANDLE;
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.sky_layer_oit_pipeline[i].handle, NULL);
		vulkan_globals.sky_layer_oit_pipeline[i].handle = VK_NULL_HANDLE;
	}
	...
	vkDestroyPipeline (vulkan_globals.device, vulkan_globals.sky_box_oit_pipeline.handle, NULL);
	vulkan_globals.sky_box_oit_pipeline.handle = VK_NULL_HANDLE;
	for (i = 0; i < MODEL_PIPELINE_COUNT; ++i)
	{
		...
		if (vulkan_globals.alias_oit_pipelines[i].handle != VK_NULL_HANDLE)
		{
		...
		if (vulkan_globals.alias_wavelet_coeff_pipelines[i].handle != VK_NULL_HANDLE)
		{
			...
			vulkan_globals.alias_wavelet_coeff_pipelines[i].handle = VK_NULL_HANDLE;
		}
		if (vulkan_globals.alias_wavelet_shade_pipelines[i].handle != VK_NULL_HANDLE)
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.alias_wavelet_shade_pipelines[i].handle, NULL);
			vulkan_globals.alias_wavelet_shade_pipelines[i].handle = VK_NULL_HANDLE;
		}
		...
		if (vulkan_globals.md5_oit_pipelines[i].handle != VK_NULL_HANDLE)
		{
		...
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.md5_wavelet_coeff_pipelines[i].handle, NULL);
			vulkan_globals.md5_wavelet_coeff_pipelines[i].handle = VK_NULL_HANDLE;
		}
		if (vulkan_globals.md5_wavelet_shade_pipelines[i].handle != VK_NULL_HANDLE)
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.md5_wavelet_shade_pipelines[i].handle, NULL);
			vulkan_globals.md5_wavelet_shade_pipelines[i].handle = VK_NULL_HANDLE;
		}
	}
	...
	if (vulkan_globals.wavelet_resolve_pipeline.handle != VK_NULL_HANDLE)
	{
	...
	if (vulkan_globals.showtris_pipeline.handle != VK_NULL_HANDLE)
	{
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.showtris_oit_pipeline.handle, NULL);
		vulkan_globals.showtris_oit_pipeline.handle = VK_NULL_HANDLE;
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.showtris_indirect_oit_pipeline.handle, NULL);
		vulkan_globals.showtris_indirect_oit_pipeline.handle = VK_NULL_HANDLE;
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.showtris_depth_test_oit_pipeline.handle, NULL);
		vulkan_globals.showtris_depth_test_oit_pipeline.handle = VK_NULL_HANDLE;
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.showtris_indirect_depth_test_oit_pipeline.handle, NULL);
		vulkan_globals.showtris_indirect_depth_test_oit_pipeline.handle = VK_NULL_HANDLE;
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.showbboxes_oit_pipeline.handle, NULL);
		vulkan_globals.showbboxes_oit_pipeline.handle = VK_NULL_HANDLE;
	}
