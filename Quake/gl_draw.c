...
qboolean GL_Set2D (void)
{
	...
	if (render_warp)
	{
		...
		const float push_constants[7] = { 1.0f / (float)vid.width, 1.0f / (float)vid.height, (float)vid.width / (float)vid.height, cl.time, (render_warp_water ? 1.0f : 0.0f), r_scale.value, inv_scale };
