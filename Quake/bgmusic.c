...
static void BGM_Loop_f (void)
{
	if (Cmd_Argc () == 2)
	{
		if (q_strcasecmp (Cmd_Argv (1), "0") == 0 || q_strcasecmp (Cmd_Argv (1), "off") == 0)
			bgmloop = false;
		else if (q_strcasecmp (Cmd_Argv (1), "1") == 0 || q_strcasecmp (Cmd_Argv (1), "on") == 0)
			bgmloop = true;
...
static void BGM_Play_noext (const char *filename, unsigned int allowed_types)
{
	...
	while (handler)
	{
		...
		q_snprintf (tmp, sizeof (tmp), "%s/%s.%s", handler->dir, filename, handler->ext);
...
void BGM_PlayCDtrack (byte track, qboolean looping)
{
	...
	while (handler)
	{
		...
		if (!CDRIPTYPE (handler->type))
			goto _next;
		q_snprintf (tmp, sizeof (tmp), "%s/track%02d.%s", MUSIC_DIRNAME, (int)track, handler->ext);
