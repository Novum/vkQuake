...
void Sky_DrawSky (cb_context_t *cbx)
{
	...
	if (flat_color)
	{
		...
		R_BindPipeline (
			...
			use_oit ? vulkan_globals.sky_color_oit_pipeline[indirect] : vulkan_globals.sky_color_pipeline[indirect]);
	...
	else if (skybox.cubemap)
	{
		R_BindPipeline (
			cbx, VK_PIPELINE_BIND_POINT_GRAPHICS,
			use_oit ? vulkan_globals.sky_cube_oit_pipeline[indirect] : vulkan_globals.sky_cube_pipeline[indirect]);
		vkCmdBindDescriptorSets (
			cbx->cb, VK_PIPELINE_BIND_POINT_GRAPHICS,
			(use_oit ? vulkan_globals.sky_cube_oit_pipeline[indirect] : vulkan_globals.sky_cube_pipeline[indirect]).layout.handle, 0, 1,
	...
	else if (!skybox.name[0])
	{
		...
		R_BindPipeline (
			cbx, VK_PIPELINE_BIND_POINT_GRAPHICS,
			use_oit ? vulkan_globals.sky_layer_oit_pipeline[indirect] : vulkan_globals.sky_layer_pipeline[indirect]);
		VkDescriptorSet descriptor_sets[2] = {solidskytexture->descriptor_set, alphaskytexture->descriptor_set};
		vkCmdBindDescriptorSets (
			cbx->cb, VK_PIPELINE_BIND_POINT_GRAPHICS,
			(use_oit ? vulkan_globals.sky_layer_oit_pipeline[indirect] : vulkan_globals.sky_layer_pipeline[indirect]).layout.handle, 0, 2,
	...
	else
	{
		R_BindPipeline (
			cbx, VK_PIPELINE_BIND_POINT_GRAPHICS,
			use_oit ? vulkan_globals.sky_stencil_oit_pipeline[indirect] : vulkan_globals.sky_stencil_pipeline[indirect]);
		R_PushConstants (cbx, VK_SHADER_STAGE_ALL_GRAPHICS, 0, 20 * sizeof (float), constant_values);
	}
	...
	if (indirect)
	{
		...
		if (flat_color)
			R_BindPipeline (
				cbx, VK_PIPELINE_BIND_POINT_GRAPHICS, use_oit ? vulkan_globals.sky_color_oit_pipeline[0] : vulkan_globals.sky_color_pipeline[0]);
		else if (skybox.cubemap)
			R_BindPipeline (
				cbx, VK_PIPELINE_BIND_POINT_GRAPHICS, use_oit ? vulkan_globals.sky_cube_oit_pipeline[0] : vulkan_globals.sky_cube_pipeline[0]);
		else if (!skybox.name[0])
			R_BindPipeline (
				cbx, VK_PIPELINE_BIND_POINT_GRAPHICS, use_oit ? vulkan_globals.sky_layer_oit_pipeline[0] : vulkan_globals.sky_layer_pipeline[0]);
		else
			R_BindPipeline (
				cbx, VK_PIPELINE_BIND_POINT_GRAPHICS, use_oit ? vulkan_globals.sky_stencil_oit_pipeline[0] : vulkan_globals.sky_stencil_pipeline[0]);
	}
	else
	...
	{
		R_BindPipeline (cbx, VK_PIPELINE_BIND_POINT_GRAPHICS, use_oit ? vulkan_globals.sky_box_oit_pipeline : vulkan_globals.sky_box_pipeline);
